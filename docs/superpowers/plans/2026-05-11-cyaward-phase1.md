...
**Goal:** Build and validate a vote-share regression model that predicts BBWAA Cy Young voting on 2015-2025 historical data via leave-one-year-out CV; gate on three-tier KPIs (winner ≥15/20, podium ≥1.9/3, top-10 ≥7/10).
...
│   │   └── training_2015_2025.parquet        # Generated by Task 7
├── src/
│   ├── __init__.py
...
**Phase 1 (current):** train + backtest model on 2015-2025 BBWAA results.
...
def test_kpi_targets_match_spec():
    assert config.KPI_TARGETS["winner_hits_min"] == 15
    assert config.KPI_TARGETS["winner_hits_total"] == 20
...
- Final line: `Saved N rows -> data/historical/training_2015_2025.parquet` where N >= 1200
...
python -c "
import pandas as pd
df = pd.read_parquet('data/historical/training_2015_2025.parquet')
print(f'Rows: {len(df)}')
...
```bash
git add data/historical/training_2015_2025.parquet
git commit -m "data: training parquet 2015-2025 (excl 2020)"
```
...
|---|---|
| Winner hits low (< 15/20) but podium/top10 OK | Model knows the elite group but can't pick the winner. Try: (a) interaction features (e.g., `fWAR × team_winning_pct`); (b) increase GBR `n_estimators`; (c) try ensemble of GBR + Ridge (average predictions). |
...
- **Phase 1 (current):** model trained + backtested on 2015-2025 BBWAA voting → KPIs in [reports/backtest_v1.md](reports/backtest_v1.md)
- **Phase 2 (deferred):** live 2026 dashboard (see [design spec](docs/superpowers/specs/2026-05-11-cyaward-design.md) section 2)

## What this is

A regression model that learns BBWAA voter behavior from historical Cy Young voting (2015-2025 excluding 2020) and predicts vote shares from pitcher stats. The model uses 38 features spanning traditional stats (W, ERA, IP), sabermetrics (fWAR, FIP, K-BB%), Statcast (xERA, Stuff+, Barrel%), and team context (winning %, run support).

It is validated against three tiers of accuracy:
- 🥇 Winner hits ≥ 15/20 (correctly identify Cy Young winner in ≥ 75% of year-league cases)
...
- **Lahman lag:** pybaseball's bundled Lahman snapshot may not include the most recent season's awards. If `data/historical/training_2015_2025.parquet` is missing 2024 winners, fetch fails loudly with a warning. Workaround: scrape baseball-reference (not yet implemented).
...
1. ☐ `data/historical/training_2015_2025.parquet` exists, ≥ 1200 rows
   ```bash
   python -c "import pandas as pd; df = pd.read_parquet('data/historical/training_2015_2025.parquet'); print(len(df))"
   ```
